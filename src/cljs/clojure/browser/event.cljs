...
(defn unique-id [identifier]); ⇒ string
(defn has-listener [obj opt_type opt_capture]); ⇒ boolean
(defn listen-with-wrapper [src wrapper listener opt_capt opt_handler])
;; TODO? (defn protect-browser-event-entry-point [errorHandler])
(defn remove-all [opt_obj opt_type opt_capt]); ⇒ number
(defn unlisten-with-wrapper [src wrapper listener opt_capt opt_handler])
